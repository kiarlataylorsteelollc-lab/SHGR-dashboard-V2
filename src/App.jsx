...
const TARGETS = {
  ...
  goals: {
    skateHausMonthly: 50000,
    giantRocket6M: 6000000,
    salesMonthly: 65000,
    grantGoalMay: 1000000,
    loungeAsk: 250000,
  },
  families: { annual: 7000, kidsServed: 3000, quarterlyBus: 300 },
  membership: { price: 150 },
  weekly: { grantsHours: 8, grantsMinimum: 2, marketingHours: 3, meetingHours: 1, dibbsSamHours: 8 },
  grossMargin: 0.5,
};

// ============== SENSORY + STEM CONCEPT (From Live Dashboard) ==============
...
// ============== DASHBOARD METRICS FORMULAS (From Excel) ==============
...
// ============== WEEKLY REVIEW QUESTIONS (From Excel) ==============
const WEEKLY_REVIEW = [
...
// ============== FULL 12-MONTH CALENDAR (From Marketing Chart) ==============
const CALENDAR_2026 = {
  jan: {
    month: 'January 2026',
    theme: '"Make Paying Easy" — 4 Big Buttons + Membership',
    primaryOffers: ['PREPAY bundles', 'Party Deposits', 'Membership + Loyalty', 'Merch/Skates'],
    keyEvents: ['Launch 4 Big Buttons', 'Start weekly Sensory Skate', 'Start biweekly Field Trips'],
    contentPillars: ['Behind-the-scenes setup', 'Family fun', '"How to prepay" demos', 'Staff intros'],
    prPartnerships: ['Schools list build', 'Sponsor prospect list', 'Community org introductions'],
    grantActions: ['Set baseline metrics', 'Build survey forms', 'Start attendance + story capture'],
    kpis: ['Revenue target $ (month)', 'Leads target (sponsors)', 'Email list growth'],
    owners: { marketing: 'Website buttons', sales: 'Sponsor list', grants: 'Baseline metrics', kiarla: 'Sensory Skate setup' }
  },
  feb: {
    month: 'February 2026',
    theme: 'Love + Belonging — families + inclusion',
    primaryOffers: ['Valentine family packs', 'Parties', 'Membership drive', 'Sponsor kit push'],
    keyEvents: ['Open House #1 (Giant Rocket)', 'Soft Launch/Media Week (Feb 17-21)', 'B2B Sponsorship Event (Feb 15)', 'Transportation partners locked (Feb 1)'],
    contentPillars: ['Parent testimonials', 'Sensory highlights', 'Party reels', 'Prepay convenience'],
    prPartnerships: ['Pitch local TV/radio', 'Autism orgs', 'School administrators'],
    grantActions: ['Capture testimonials + signups', 'Draft 2 grant applications'],
    kpis: ['Revenue target $', 'Memberships #', 'Sponsor meetings #'],
    owners: { kiarla: 'Open House #1 PowerPoint/agenda', marketing: 'Valentine promos', sales: 'Sponsor kit push', votp: 'Media Week coverage' }
  },
  mar: {
    month: 'March 2026',
    theme: 'Community Momentum — "314" + school pipeline',
    primaryOffers: ['Membership + Sensory signups', 'Field Trips', 'Party deposits'],
    keyEvents: ['Open House #2 (Mar 14 / 314 Day)', 'Spring Break promo'],
    contentPillars: ['Event recap', 'Countdown to Apr 1', 'Student field trip stories'],
    prPartnerships: ['Partnership MOUs', 'Sponsor LOIs', 'PTAs + districts'],
    grantActions: ['Finalize Autism Month impact plan', 'Confirm data collection'],
    kpis: ['Revenue target $', 'Field trip bookings #', 'Sponsors committed #'],
    owners: { kiarla: 'Open House #2', assistant: 'Intake pipeline setup', marketing: 'Countdown content', sales: 'LOIs' }
  },
  apr: {
    month: 'April 2026',
    theme: 'Autism Awareness Month — proof + press + funding',
    primaryOffers: ['Giant Rocket memberships', 'Sponsorships', 'Fundraiser tickets', 'Donor asks'],
    keyEvents: ['🚀 Apr 1 Kickoff', 'Apr 2 Awareness Day press', 'Major Fundraiser'],
    contentPillars: ['Daily impact stories', 'Sensory zone visuals', 'Staff training', 'Family wins'],
    prPartnerships: ['Press blitz', 'Elected/community leaders', 'Corporate sponsors'],
    grantActions: ['Grant submissions', 'Reporting-ready proof', 'Compile surveys'],
    kpis: ['Revenue target $', 'Fundraiser $', 'Grant $ submitted'],
    owners: { votp: 'Press blitz + daily stories', kiarla: 'Fundraiser', grants: 'Grant submissions', sales: 'Sponsorships' }
  },
  may: {
    month: 'May 2026',
    theme: 'Convert Attention → Paid Programs',
    primaryOffers: ['Camps', 'Afterschool', 'Family packs', 'Parties', 'Merch'],
    keyEvents: ['Camp/afterschool enrollment push', 'Memorial Day events', '🎯 $1M GRANT TARGET'],
    contentPillars: ['Program explainers', 'Parent FAQs', '"What\'s included" content'],
    prPartnerships: ['Schools for summer', 'Youth orgs', 'Corporate volunteer days'],
    grantActions: ['Follow-ups on grants', 'Mid-program evaluation snapshots'],
    kpis: ['Revenue target $', 'Enrollments #', 'Merch $', '🎯 $1M GRANT TARGET'],
    owners: { grants: 'Grant sprint lead', sales: 'Camp enrollments', marketing: 'Program explainers', kiarla: 'Memorial Day' }
  },
  jun: {
    month: 'June 2026',
    theme: 'Close the Half-Year — sponsor summit + proof showcase',
    primaryOffers: ['Sponsor renewals', 'Multi-year commitments', 'Memberships'],
    keyEvents: ['Sponsor Summit + Proof Showcase', 'June 30 closeout', '🎯 $6M TARGET CHECK'],
    contentPillars: ['Sponsor spotlight', 'Year-to-date impact', 'Behind the scenes'],
    prPartnerships: ['Sponsor renewal calls', 'Grant officers invited to showcase'],
    grantActions: ['Submit final June grants', 'Reconcile proof/metrics', 'Deliver VOTP assets'],
    kpis: ['Revenue target $', 'Sponsorship $', '🎯 YTD toward $6M'],
    owners: { sales: 'Sponsor summit + renewals', assistant: 'Mid-year impact report', kiarla: 'Quarterly bus', marketing: 'Summer bundles' }
  },
  jul: {
    month: 'July 2026',
    theme: 'Summer Energy',
    primaryOffers: ['Camps', 'Parties', 'Family passes', 'Merch'],
    keyEvents: ['Summer theme nights', 'Community collabs'],
    contentPillars: ['Fun reels', 'Camp highlights', 'Staff spotlights'],
    prPartnerships: ['Tourism + city calendars', 'Youth orgs'],
    grantActions: ['Collect summer outcomes', 'Prep fall grant cycle'],
    kpis: ['Revenue target $', 'Attendance #'],
    owners: { kiarla: 'Theme nights', sales: '5 new sponsor meetings', marketing: 'Camp highlights' }
  },
  aug: {
    month: 'August 2026',
    theme: 'Back-to-School Prep',
    primaryOffers: ['Afterschool', 'Field trips', 'Memberships'],
    keyEvents: ['School outreach wave', 'Afterschool signups', 'Quarterly Bus #2 (300 families)'],
    contentPillars: ['Teacher/school content', 'Program benefits'],
    prPartnerships: ['Districts + PTAs', 'Special ed coordinators'],
    grantActions: ['Fall grant shortlist', 'Letters of support'],
    kpis: ['Bookings #', 'Leads #'],
    owners: { kiarla: 'Quarterly bus', sales: 'School outreach', grants: 'Fall grant prep', marketing: 'Teacher content' }
  },
  sep: {
    month: 'September 2026',
    theme: 'Fall Kickoff + Inclusion',
    primaryOffers: ['Afterschool launch', 'Sponsor activations'],
    keyEvents: ['Quarterly Impact Showcase (optional)'],
    contentPillars: ['Impact stories', 'Routine + belonging'],
    prPartnerships: ['Local news "community" segment'],
    grantActions: ['Grant submissions (fall cycle)'],
    kpis: ['Revenue $', 'Donors #'],
    owners: { kiarla: 'Afterschool launch + staff training', grants: 'Fall submissions', marketing: 'Impact stories' }
  },
  oct: {
    month: 'October 2026',
    theme: 'Community + Harvest + Awareness',
    primaryOffers: ['Halloween events', 'Family packs', 'Sponsor nights'],
    keyEvents: ['Halloween skate', 'Costume nights', 'Community Impact Summit'],
    contentPillars: ['Seasonal promo', 'Safety + family'],
    prPartnerships: ['Community calendars', 'Brands for co-sponsor'],
    grantActions: ['Data compilation for year-end'],
    kpis: ['Revenue $', 'Sponsor $'],
    owners: { kiarla: 'Halloween + Summit', votp: 'Earned media', assistant: 'Data compilation', sales: 'Co-sponsors' }
  },
  nov: {
    month: 'November 2026',
    theme: 'Giving Season',
    primaryOffers: ['Donor drive', 'Sponsor packages', 'Gift cards'],
    keyEvents: ['Giving Tuesday campaign', 'Family events'],
    contentPillars: ['Donor stories', '"Why we give"'],
    prPartnerships: ['Pitch philanthropy media', 'Foundations'],
    grantActions: ['Submit year-end grant asks'],
    kpis: ['Donations $', 'New donors #'],
    owners: { sales: 'Giving Tuesday + renewals', kiarla: 'Holiday events', grants: 'Year-end asks', marketing: '"Why we give" content' }
  },
  dec: {
    month: 'December 2026',
    theme: 'Year-End Close + Renewal',
    primaryOffers: ['Membership renewals', 'Parties', 'Gift cards', 'Merch'],
    keyEvents: ['Holiday events', 'Year-end recap'],
    contentPillars: ['Recap montage', 'Gratitude', 'Impact metrics'],
    prPartnerships: ['Sponsor renewals', 'Media recap'],
    grantActions: ['Grant reports', 'Next year pipeline'],
    kpis: ['Revenue $', 'Renewals #'],
    owners: { kiarla: 'Holiday events', sales: 'Sponsor renewals', grants: 'Reports + pipeline', votp: 'Recap montage' }
  }
};

// ============== CAMPAIGN GANTT (From Marketing Chart) ==============
...
// ============== WEEKLY CONTENT CADENCE (From Marketing Chart) ==============
...
// ============== KPI TARGETS JAN-JUN (From Marketing Chart) ==============
const KPI_TARGETS = [
...
// ============== MACRO/MICRO PLANNER (From Excel) ==============
const MACRO_MICRO_TASKS = [
  { level: 'MACRO', area: 'Content', task: 'Build weekly VOTP content pack (draft captions + asset list)', owner: 'Social', frequency: 'Weekly', priority: 'High' },
  { level: 'MACRO', area: 'Grants', task: 'Submit 2 grants (1 big, 1 small) + set follow-ups', owner: 'Grants', frequency: 'Weekly', priority: 'High' },
  { level: 'MICRO', area: 'Ops', task: 'Update Daily Inputs in dashboard', owner: 'Admin', frequency: 'Daily', priority: 'Med' },
];

// ============== ALL ROLE TASKS (Combined from all documents) ==============
...
function App() {
  ...
  const [kpiTargets, setKpiTargets] = useState(KPI_TARGETS);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-800 text-white">
                ...
                <th className="p-2 text-left">Key Actions</th>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="p-2 text-left">Input Sources</th>
            ...
            <tbody>
              {DASHBOARD_METRICS.map((m, i) => (
                <tr key={i} className="border-b">
                  ...
                  <td className="p-2 text-gray-600">{m.source}</td>
                  <td className="p-2">{m.reviewer}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="p-2">Skate Haus Rev $</th>
                <th className="p-2">Giant Rocket Funding $</th>
                <th className="p-2">Other Rev $</th>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 bg-white p-2 rounded-xl shadow">
        {Object.entries(CALENDAR_2026).map(([key, data]) => (
          <button key={key} onClick={() => setSelectedMonth(key)} className={`px-3 py-1 rounded text-sm font-medium ${selectedMonth === key ? 'bg-purple-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}>
      ...
      {CALENDAR_2026[selectedMonth] && (
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="p-4 grid md:grid-cols-2 gap-4">
            <div>
              <h3 className="font-bold text-purple-700 mb-2">🛒 Primary Offers</h3>
              <div className="flex flex-wrap gap-1">{CALENDAR_2026[selectedMonth].primaryOffers.map((o, i) => <span key={i} className="bg-purple-100 px-2 py-1 rounded text-xs">{o}</span>)}</div>
            </div>
            <div>
              <h3 className="font-bold text-blue-700 mb-2">🎉 Key Events</h3>
              <ul className="text-sm space-y-1">{CALENDAR_2026[selectedMonth].keyEvents.map((e, i) => <li key={i}>• {e}</li>)}</ul>
            </div>
            <div>
              <h3 className="font-bold text-green-700 mb-2">📱 Content Pillars</h3>
              <div className="flex flex-wrap gap-1">{CALENDAR_2026[selectedMonth].contentPillars.map((c, i) => <span key={i} className="bg-green-100 px-2 py-1 rounded text-xs">{c}</span>)}</div>
            </div>
            <div>
              <h3 className="font-bold text-orange-700 mb-2">🤝 PR + Partnerships</h3>
              <ul className="text-sm space-y-1">{CALENDAR_2026[selectedMonth].prPartnerships.map((p, i) => <li key={i}>• {p}</li>)}</ul>
            </div>
            <div>
              <h3 className="font-bold text-yellow-700 mb-2">📝 Grant Actions</h3>
              <ul className="text-sm space-y-1">{CALENDAR_2026[selectedMonth].grantActions.map((g, i) => <li key={i}>• {g}</li>)}</ul>
            </div>
            <div>
              <h3 className="font-bold text-red-700 mb-2">📊 KPIs</h3>
              <div className="flex flex-wrap gap-1">{CALENDAR_2026[selectedMonth].kpis.map((k, i) => <span key={i} className="bg-red-100 px-2 py-1 rounded text-xs font-medium">{k}</span>)}</div>
            </div>
            <div className="md:col-span-2">
              <h3 className="font-bold text-gray-700 mb-2">👤 Owners</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {Object.entries(CALENDAR_2026[selectedMonth].owners).map(([role, task]) => (
                  <div key={role} className="bg-gray-100 p-2 rounded text-xs"><span className="font-bold uppercase">{role}:</span> {task}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-gray-800 text-white">
              ...
              {['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map(m => <th key={m} className="p-1">{m}</th>)}
            </tr>
          </thead>
          <tbody>
            {CAMPAIGN_GANTT.initiatives.map((item, i) => (
              <tr key={i} className="border-b hover:bg-gray-50">
                ...
                {['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec'].map(m => (
                  <td key={m} className="p-1 text-center"><PriorityBadge level={item[m]} /></td>
                ))}
              </tr>
            ))}
          </tbody>
      ...
      <div className="bg-white rounded-xl shadow-lg p-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-800 text-white">
                <th className="p-2">Day</th><th className="p-2">SH Post</th><th className="p-2">GR Post</th><th className="p-2">VOTP?</th><th className="p-2">Email/SMS</th><th className="p-2">Outreach</th><th className="p-2">Notes</th><th className="p-2">Status</th>
              </tr>
            </thead>
            <tbody>
              {contentPack.map(item => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2 text-center">
                    <button onClick={() => canApproveContent && cycleContentStatus(item.id)} disabled={!canApproveContent} className={canApproveContent ? 'cursor-pointer' : 'opacity-50'}>
      ...
      <div className="bg-white rounded-xl shadow-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 text-sm">
          ...
          <div className="flex items-center gap-2 col-span-2">
            <label className="flex items-center gap-1 text-xs"><input type="checkbox" checked={newLog.sensoryAdaptive} onChange={e => setNewLog({...newLog, sensoryAdaptive: e.target.checked})} /> Sensory?</label>
            <label className="flex items-center gap-1 text-xs"><input type="checkbox" checked={newLog.photosUploaded} onChange={e => setNewLog({...newLog, photosUploaded: e.target.checked})} /> Photos?</label>
          </div>
              ...
              <tbody>
                {dailyLogs.slice(0, 20).map(log => (
                  <tr key={log.id} className="border-b">
                    ...
                    <td className="p-1">${log.admissions}</td>
                    <td className="p-1">${log.rentals}</td>
                    <td className="p-1">${log.concessions}</td>
                    <td className="p-1">{log.uniqueVisitors}</td>
                    <td className="p-1">{log.membershipsSold}</td>
                    <td className="p-1">{log.partiesBooked}</td>
                    <td className="p-1">{log.grFamilies}</td>
                    <td className="p-1">${log.sponsorDollars}</td>
                    <td className="p-1">${log.grantDollars}</td>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-purple-100 to-blue-100 rounded-xl p-4 border-2 border-purple-300">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-xs text-purple-600 font-semibold">{CALENDAR_2026[selectedMonth].month}</p>
            <h2 className="text-lg font-bold text-purple-800">{CALENDAR_2026[selectedMonth].theme}</h2>
          </div>
          <div className="text-right text-xs">
            <p className="font-bold">${TARGETS.goals.skateHausMonthly.toLocaleString()}/mo SH</p>
            <p className="font-bold">${TARGETS.goals.grantGoalMay.toLocaleString()} grants by May</p>
            <p className="font-bold text-purple-700">$6M GR by June</p>
          </div>
        </div>
      </div>
      {isCEO && (
      ...
      <div className="bg-white rounded-xl shadow-lg p-4">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          <div className="bg-blue-50 p-2 rounded">
            <p className="text-xs font-bold text-blue-700 mb-1">DAILY</p>
            {userTasks.daily.map((t, i) => <CheckItem key={i} small checked={completedTasks[`my-d-${i}`]} onClick={() => toggleTask(`my-d-${i}`)}>{t}</CheckItem>)}
          </div>
          <div className="bg-green-50 p-2 rounded">
            <p className="text-xs font-bold text-green-700 mb-1">WEEKLY</p>
            {userTasks.weekly.map((t, i) => <CheckItem key={i} small checked={completedTasks[`my-w-${i}`]} onClick={() => toggleTask(`my-w-${i}`)}>{t}</CheckItem>)}
          </div>
          <div className="bg-purple-50 p-2 rounded">
            <p className="text-xs font-bold text-purple-700 mb-1">MONTHLY</p>
            {userTasks.monthly.map((t, i) => <CheckItem key={i} small checked={completedTasks[`my-m-${i}`]} onClick={() => toggleTask(`my-m-${i}`)}>{t}</CheckItem>)}
          </div>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow p-3">
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-xs">
          {[{d:'MON',l:'PLAN',c:'#dbeafe'},{d:'TUE',l:'PEOPLE',c:'#dcfce7'},{d:'WED',l:'GRANTS',c:'#fef9c3'},{d:'THU',l:'CONTENT',c:'#ffedd5'},{d:'FRI',l:'MONEY',c:'#fee2e2'},{d:'SAT',l:'POST',c:'#f3e8ff'},{d:'SUN',l:'RESET',c:'#f3f4f6'}].map(x => (
            <div key={x.d} className="p-2 rounded" style={{backgroundColor:x.c}}><p className="font-bold">{x.d}</p><p>{x.l}</p></div>
          ))}
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-purple-50 p-2 rounded">
              ...
              <div className="space-y-1">
                <input type="number" placeholder="Admissions $" className="w-full border rounded p-1" value={scoreboard.sh.admissions || ''} onChange={e => saveScoreboard({...scoreboard, sh: {...scoreboard.sh, admissions: parseInt(e.target.value) || 0}})} />
                <input type="number" placeholder="Rentals $" className="w-full border rounded p-1" value={scoreboard.sh.rentals || ''} onChange={e => saveScoreboard({...scoreboard, sh: {...scoreboard.sh, rentals: parseInt(e.target.value) || 0}})} />
                <input type="number" placeholder="Memberships #" className="w-full border rounded p-1" value={scoreboard.sh.membershipsSold || ''} onChange={e => saveScoreboard({...scoreboard, sh: {...scoreboard.sh, membershipsSold: parseInt(e.target.value) || 0}})} />
                <input type="number" placeholder="Parties Booked" className="w-full border rounded p-1" value={scoreboard.sh.partiesBooked || ''} onChange={e => saveScoreboard({...scoreboard, sh: {...scoreboard.sh, partiesBooked: parseInt(e.target.value) || 0}})} />
              </div>
            </div>
            <div className="bg-blue-50 p-2 rounded">
              <p className="font-bold text-blue-700">🚀 GIANT ROCKET</p>
              <input type="number" placeholder="Families" className="w-full border rounded p-1 mt-1" value={scoreboard.gr.families || ''} onChange={e => saveScoreboard({...scoreboard, gr: {...scoreboard.gr, families: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="New Members" className="w-full border rounded p-1 mt-1" value={scoreboard.gr.newMembers || ''} onChange={e => saveScoreboard({...scoreboard, gr: {...scoreboard.gr, newMembers: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="Testimonials" className="w-full border rounded p-1 mt-1" value={scoreboard.gr.testimonials || ''} onChange={e => saveScoreboard({...scoreboard, gr: {...scoreboard.gr, testimonials: parseInt(e.target.value) || 0}})} />
            </div>
            <div className="bg-green-50 p-2 rounded">
              <p className="font-bold text-green-700">💰 SALES</p>
              <input type="number" placeholder="Contacted" className="w-full border rounded p-1 mt-1" value={scoreboard.sales.contacted || ''} onChange={e => saveScoreboard({...scoreboard, sales: {...scoreboard.sales, contacted: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="Meetings" className="w-full border rounded p-1 mt-1" value={scoreboard.sales.meetings || ''} onChange={e => saveScoreboard({...scoreboard, sales: {...scoreboard.sales, meetings: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="Sponsor $" className="w-full border rounded p-1 mt-1" value={scoreboard.sales.sponsorDollars || ''} onChange={e => saveScoreboard({...scoreboard, sales: {...scoreboard.sales, sponsorDollars: parseInt(e.target.value) || 0}})} />
            </div>
            <div className="bg-yellow-50 p-2 rounded">
              <p className="font-bold text-yellow-700">📝 GRANTS</p>
              <input type="number" placeholder="Submitted" className="w-full border rounded p-1 mt-1" value={scoreboard.grants.submitted || ''} onChange={e => saveScoreboard({...scoreboard, grants: {...scoreboard.grants, submitted: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="Follow-ups" className="w-full border rounded p-1 mt-1" value={scoreboard.grants.followUps || ''} onChange={e => saveScoreboard({...scoreboard, grants: {...scoreboard.grants, followUps: parseInt(e.target.value) || 0}})} />
              <input type="number" placeholder="$ Awarded" className="w-full border rounded p-1 mt-1" value={scoreboard.grants.grantDollars || ''} onChange={e => saveScoreboard({...scoreboard, grants: {...scoreboard.grants, grantDollars: parseInt(e.target.value) || 0}})} />
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex gap-1">
                  {p.stages.map(s => (
                    <div key={s} className="flex-1 text-center">
                      ...
                      <input type="number" className="w-full text-center border rounded p-1 font-bold" value={pipelines[p.key][s] || 0} onChange={e => savePipelines({...pipelines, [p.key]: {...pipelines[p.key], [s]: parseInt(e.target.value) || 0}})} />
                    </div>
                  ))}
                </div>
              </div>
